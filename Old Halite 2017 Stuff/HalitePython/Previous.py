...
while True:
    ...
    for current in range(0, len(myShips)):
        ...
        for planet in game_map.all_planets():
            ...
            '''if (len(myShips) <= 3):     #initial attack stratagy
                if (moveCount <= 8):
                    if (current == 1):
                        if (len(game_map.all_players()) <= 2):
                            attack_planet = closest_planet
                            for newnewPlanet in game_map.all_planets():
                                if game_map.get_me().id != newnewPlanet.owner.id:
                                    attack_planet = newnewPlanet #declaration of random enemy planet

                            for newPlanet in game_map.all_planets():
                                if game_map.get_me().id != newPlanet.owner.id:
                                    if myShips[current].calculate_distance_between(newPlanet) < myShips[current].calculate_distance_between(attack_planet):
                                        attack_planet = newPlanet
                                        #makes the nearest enemy planet a target

                            #attack
                            ship_list_one = attack_planet.all_docked_ships()
                            attack_ship = ship_list_one[0]
                            for x in ship_list_one:
                                if(myShips[current].calculate_distance_between(x) < myShips[current].calculate_distance_between(attack_ship)):
                                    attack_ship = x
                                
                            if myShips[current].calculate_distance_between(attack_ship) > (hlt.constants.DOCK_RADIUS * 1.9):
                                moveSpd = 1
                            else:
                                moveSpd = 1.6
                            navigate_command = myShips[current].navigate(myShips[current].closest_point_to(attack_ship), game_map, speed=hlt.constants.MAX_SPEED/moveSpd, angular_step=5, ignore_ships=True, ignore_planets=False)
                            if navigate_command:
            ...
            if (len(myShips) <= 3) and (moveCount <= 20): #code to diversift first 3 planets
                ...
                for planetNew in game_map.all_planets(): #start of planet prioritizing code
                    if planetNew.is_owned():
                        newMap = game_map.all_planets()
                        newMap.remove(planetNew)
                        nextClosestPlanet = game_map.all_planets()[0]
                        for newNextPlanet in newMap:
                            if myShips[current].calculate_distance_between(newNextPlanet) < myShips[current].calculate_distance_between(nextClosestPlanet):
                                nextClosestPlanet = newNextPlanet
                        if (len(myShips) < 5) and (countF >= 2) and (myShips[current].calculate_distance_between(nextClosestPlanet) < 4.5):
