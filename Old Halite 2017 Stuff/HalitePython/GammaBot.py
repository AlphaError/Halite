...
while True:
    ...
    for current in range(0, len(myShips)):
        ...
        for planet in game_map.all_planets():
            ...
            '''if (current != 4) and (moveCount < 32):   #initial attack stratagy
                enemyShip = game_map._all_ships()[1]
            ...
            if (len(myShips) <= 3) and (moveCount <= 20): #code to diversift first 3 planets
                ...
                if (moveCount > 60) and (countF <= 1): #escape!
                    movePos = pos1
                ...
                for planetNew in game_map.all_planets(): #start of planet prioritizing code  ********************
                    if planetNew.is_owned():
                        if game_map.get_me().id == planetNew.owner.id:
                            isAttackable = False
                            if (moveCount >= 50) and (myShips[current].calculate_distance_between(closest_planet) < 1.6):
                                if closest_planet.calculate_distance_between(enemyAttack) <= (closest_planet.radius + 1.2):
                                    isAttackable = True
                            elif (moveCount >= 15) and (myShips[current].calculate_distance_between(closest_planet) <= 1.1):
                                if closest_planet.calculate_distance_between(enemyAttack) <= (closest_planet.radius + 1):
                                    isAttackable = True
                            if (isAttackable == False) and (planetNew.is_full() == True):
                                continue
                ...
                if closest_planet.is_owned():
                    if game_map.get_me().id == closest_planet.owner.id:  #attack and defense
                        #enemyAttack **
                        if (moveCount >= 50) and (myShips[current].calculate_distance_between(closest_planet) < 1.6): #DEFENSE
                            #enemyAttack
                            if closest_planet.calculate_distance_between(enemyAttack) <= (closest_planet.radius + 1.2): #spawn radius plus ...
                                if myShips[current].calculate_distance_between(enemyAttack) > (hlt.constants.DOCK_RADIUS*1.5):
                                    moveSpd = 1 #persuit
                                else:
                                    moveSpd = 1.3
                                navigate_command = myShips[current].navigate(myShips[current].closest_point_to(enemyAttack), game_map, speed=hlt.constants.MAX_SPEED/moveSpd, angular_step=11, ignore_ships=True, ignore_planets=False)
                                if navigate_command:
                                    command_queue.append(navigate_command)
                                break
                        elif (moveCount >= 15) and (myShips[current].calculate_distance_between(closest_planet) <= 1.1): #DEFENSE
                            #enemyAttack
                            if closest_planet.calculate_distance_between(enemyAttack) <= (closest_planet.radius + 1): #spawn radius plus ...
                                if myShips[current].calculate_distance_between(enemyAttack) > (hlt.constants.DOCK_RADIUS*1.5):
                                    moveSpd = 1 #persuit
                                else:
                                    moveSpd = 1.3
                                navigate_command = myShips[current].navigate(myShips[current].closest_point_to(enemyAttack), game_map, speed=hlt.constants.MAX_SPEED/moveSpd, angular_step=12, ignore_ships=True, ignore_planets=False)
                                if navigate_command:
                                    command_queue.append(navigate_command)
                                break
                        elif closest_planet.is_full() == True: #extra precausion
                            continue
                    if (game_map.get_me().id != closest_planet.owner.id):
                        ...
                        if myShips[current].calculate_distance_between(newShip) > (hlt.constants.DOCK_RADIUS * 2):
                            ...
                            moveSpd = 1.5
                        navigate_command = myShips[current].navigate(myShips[current].closest_point_to(newShip), game_map, speed=hlt.constants.MAX_SPEED/moveSpd, angular_step=9, ignore_ships=False, ignore_planets=False)
                        if navigate_command:
                            command_queue.append(navigate_command)
                        break
